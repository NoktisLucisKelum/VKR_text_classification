...
def testing_tfidf_upgrade(name: str):
    for i in dict_of_frames.keys():
        ...
        print(
            f'Тип датасета: {name} Датасет: {i}, Векторизатор: TF-IDF, F1 Score: {f1:.3f}, Длина датасета: {len(dict_of_frames[i][0])},'
            f'Количество классов в датасете:  {len(dict_of_frames[i][0]['RGNTI3'].unique().tolist())},'
            f'  Тема: {dict_of_frames[i][1]}')

testing_tfidf_upgrade("Обычный")

df = pd.read_csv(
    '/datasets/datasets_final/for_1_level/train_refactored_lematize_no_numbers.csv',
    dtype={'RGNTI1': str, 'RGNTI2': str, 'RGNTI3': str})
...
testing_tfidf_upgrade("без чисел")